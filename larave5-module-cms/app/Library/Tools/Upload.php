...
class Upload
{
    ...
    //文件上传
    public static function upload(Request $request, $fileName, $configKey)
    {
        try {
            ...
            if ($request->hasFile($fileName)) {
                ...
                if ($pic->isValid()) {
                    $path    = config("apps.uploads.".self::$configKey.".connections.local.prefix", 'uploads');
                    ...
                    //同步到七牛
                    if (in_array('qiniu', config("apps.uploads.".self::$configKey.".dirver", []))) {
                        self::_qiniu();
                    }
                    //同步到阿里云
                    if (in_array('aliyun', config("apps.uploads.".self::$configKey.".dirver", []))) {
                        self::_aliyun();
                    }
                } else {
    ...
    {
        try {
            $qiniu = config("apps.uploads.".self::$configKey.".connections.qiniu");
    ...
    {
        try {
            $aliyun = config("apps.uploads.".self::$configKey.".connections.aliyun");
