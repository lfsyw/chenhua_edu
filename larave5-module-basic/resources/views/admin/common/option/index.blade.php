...
@section('content')
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-2 text-right p-t-10">
            <a class="btn btn-info btn-md" href="{{route('option.create')}}">添加配置项</a>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    ...
                    <form action="{{url('admin/option/updateValue')}}" method="POST" class="form-horizontal">
                    {{csrf_field()}}
                        <div class="ibox-content">
                        @forelse($options as $option)
                            <div class="form-group">
                                <label class="col-sm-2 control-label">{{$option->title}} <a href="{{route('option.edit',['id'=>$option->id])}}"><i class="fa fa-edit"></i></a> <br/>{{$option->name}}</label>
                                <div class="col-sm-10">
                                    {!! $option->html !!}
