...
@section('content')
    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-2 text-right p-t-10">
            <a class="btn btn-info btn-md" href="{{route('option.index')}}">配置项列表</a>
        </div>
    </div>
            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form method="POST" class="form-horizontal" action="{{route('option.update',['id'=>$field->id])}}">
                            {{method_field('PUT')}}
                            {{csrf_field()}}
                            <div class="form-group @if($errors->has('title')) has-error @endif">
                                <label class="col-sm-2 control-label">中文名</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="title" value="{{old('title',$field->title)}}">
                                </div>
                            </div>
                                    ...
                                    @foreach($fieldType as $type)
                                        <label class="checkbox-inline">
                                            <input type="radio" name="field_type" value="{{$type}}" onclick="changeType()"
                                                   @if(old('field_type',$field->field_type)==$type) checked @endif
                                            > {{$type}}
                                        </label>
                                    @endforeach
                                </div>
                            ...
                            <div class="field_value" style="display:none;">
                                <div class="form-group @if($errors->has('field_value')) has-error @endif">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea class="form-control" name="field_value">{{old('field_value',$field->field_value)}}</textarea>
                            ...
                            <div class="form-group @if($errors->has('order')) has-error @endif">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="order" value="{{old('order',$field->order)}}">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group @if($errors->has('remark')) has-error @endif">
                                ...
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="remark">{{old('remark',$field->remark)}}</textarea>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                <div class="col-sm-10 col-sm-offset-2">
                                    ...
                                    <button class="btn btn-danger pull-right" type="button" onclick="delData(this,{{$field->id}})">删除</button>
                                </div>
                            </div>
                        </form>
...
@section('footer')
    ...
    @include('admin.includes._delete',[
        'beseUrl'=>route('option.index'),
        'jumpUrl'=>route('option.index')
    ])
@endsection